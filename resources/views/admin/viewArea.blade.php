		...
		<div class="page-title-right">
				<a href="{{url('/getLevel')}}" class="btn btn-primary mb-2" >Go Back</a>
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active" style = "display:none" id = "headerShow">View/Edit areas</li>
                    </ol>
                </div>
                <h4 class="page-title">View/Edit areas</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="text-muted font-14">
                        Below are the lists of areas availabe within City Cyber. Offices can also be edited
                    </p>
                                    ...
                                    @foreach($areas as $area)
                                        <tr>
                                           <!-- <td>{{$area->created_at}}</td>-->
                                            <td>{{$area->name}}</td>
					    <td>{{$area->location}}</td>
					     <td>{{$area->area_acronym}}</td>
                                            <td>{{$area->state_id ?? 'none'}}</td>
                                            <td>{{$area->city_id ?? 'none'}}</td>
                                            <td>
                                                <form method="get" action="{{url('officeInfo')}}">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{$area->id}}">
                                                    <input type="hidden" name="description" value="{{$area->name}}">
                                                    <button name = "submit" value = "edit" class="btn btn-primary btn-sm"><span class="uil-eye"></span> View </button>
                                                </form>
                                          <!--      <a href="{{route('viewAddPhotos', ['officeid'=>$area->id])}}" class="btn btn-primary btn-sm mt-1"><span class="uil-wallet"></span> Upload Photos</a>-->

                                                @if(!$area->has_store)
                                                    <a href="{{route('office.createStore', ['id'=>$area->id])}}" class="btn btn-primary btn-sm mt-1"><span class="uil-wallet"></span> Create Store</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
