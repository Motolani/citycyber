    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <a name="submit" value="edit" class="btn btn-primary"  href="{{route('create.staff.deduction')}}"><span class="uil-plus"></span> Create New Deduction</a>                

                <p style="margin-top: 10px" class="text-muted font-14">
                    Below are Deductions made within City Cyber
                </p>
                ...
                <div class="tab-content">
                    <div class="tab-pane show active" id="buttons-table-preview">
                        <table id="datatable-buttons" class="table data-table table-striped dt-responsive nowrap w-100">
                            ...
                            <tbody>
                                @if(isset($deductionoperations))
                                {{-- @dd($deductionoperations) --}}
                                @foreach($deductionoperations as $stat)
                                @php
                                    $deductDetails = \App\Deduction::where('id', $stat->deduction_id)->first();
                                    $deduction_name = $deductDetails->deduction;
                                    $staffDetails = \App\User::where('id', $stat->staff_id)->first();
                                    $staffName = $staffDetails->firstname . " " . $staffDetails->lastname;
                                    // var_dump($deduction_name);
                                @endphp
                                <tr>
                                    ...
                                    <div class="tab-content">
                                                    ...
                                                    <div class="modal-dialog modal-top">
                                                        <div class="modal-content">
                                                            ...
                                                            <form class="ps-3 pe-3"
                                                                action="{{url('updatededuction/' . $stat->id)}}"
                                                                method="POST">
                                                                @csrf
                                                                <div class="mb-3">
                                                                    
                                                                    <label for="example-email" class="form-label">Deduction</label>
                                                                    <input type="text" id="example-email" name="deduction" value={{$deductDetails->deduction}} class="form-control" placeholder="Enter deduction" required>
                                                                </div>

                                                                <div class="mb-3">
                                                                    <label for="example-password" class="form-label">Amount</label>
                                                                    <input type="text" id="example-amount" name="amount" value={{$stat->amount}} class="form-control" placeholder="Enter amount" required>
                                                                </div>

                                                                <button type="submit" name="submit"
                                                                    class="btn btn-primary mt-2 mb-2 ">UPDATE DEDUCTION
                                                                </button>
                                                            </form>
                                                        </div><!-- /.modal-content -->
                                                    </div><!-- /.modal-dialog -->
                                        ...
                                        <a onclick="return confirm('Are you sure you want to delete {{$stat->allowance}}?, this action is not be reversable!.')" class="btn btn-danger btn-sm" href="{{url('/deletededuction/' . $stat->id)}}">
                                            <i class="uil-trash"></i>
                                        </a>
                                    </td>    
                                    <td>{{$staffName}}                              
