...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title">Staff Deduction</h4>
        </div>
    </div>
</div>
<!-- end page title -->
		...
		@endif
                ...
                <button value="edit" style = "color:white" id = "createIncidence" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#edit-modal1"><span style="color: #fff"
                                                class="uil-plus"></span>Create Deduction</button>

                <p style="margin-top: 10px" class="text-muted font-14">
                    Below are the list of deductions Created
                </p>
                            ...
                            <tbody>
                                ...
                                <tr>
                                   {{-- <td>
                                    ...
                                    <td>{{isset($data->deduction)?$data->deduction:'null'}}</td>
                            ...
                            <div class="modal-dialog modal-top">
                                <div class="modal-content">
                                    ...
                                    <form class="ps-3 pe-3"
                                        action="{{url('viewCreateDeduction')}}"
                                        method="POST">
                                        @csrf
                                        <div class="mb-3">
                                            <input type="hidden" id="user_id" name="user_id" class="form-control" value="{{isset($user_id)?$user_id:'1'}}" >
                                            <label for="example-email" class="form-label">Staff Deduction</label>
                                            <select id="offe" class="form-control" name="deduction_id" data-toggle="select" required>
                                                
                                                <option value="">Select Deduction</option>
                                                @if(isset($deductionList))
                                                @foreach($deductionList as $data)
                                                <option value="{{$data->id}}">{{$data->deduction}}</option>
                                        ...
                                        <button type="submit" name="submit" value = "createDeduction"
                                            class="btn btn-primary mt-2 mb-2 ">Create Deduction
                                        </button>
                                    </form>
                                </div><!-- /.modal-content -->
                            </div><!-- /.modal-dialog -->
