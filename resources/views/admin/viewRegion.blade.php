		...
		<div class="page-title-right">
				 <a href="{{url('/getLevel')}}" class="btn btn-primary mb-2" >Go Back</a>
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active" style = "display:none" id = "headerShow">View/Edit Regions</li>
                    </ol>
                </div>
                <h4 class="page-title">View/Edit Regions</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
            ...
            <div class="card">
                <div class="card-body">
				    ...
				    <th>Region Acronym</th>
					 ...
					 <td>{{$region->region_acronym}}</td>
                                            <td>{{$region->state_id ?? 'none'}}</td>
                                            <td>{{$region->city_id ?? 'none'}}</td>
                                            <td>
                                                <form method="get" action="{{url('officeInfo')}}">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{$region->id}}">
                                                    <input type="hidden" name="description" value="{{$region->name}}">
                                                    <button name = "submit" value = "edit" class="btn btn-primary btn-sm"><span class="uil-eye"></span> View </button>
                                                </form>
                                                 <!--     <a href="{{route('viewAddPhotos', ['officeid'=>$region->id])}}" class="btn btn-primary btn-sm mt-1"><span class="uil-wallet"></span> Upload Photos</a>-->

                                                @if(!$region->has_store)
                                                    <a href="{{route('office.createStore', ['id'=>$region->id])}}" class="btn btn-primary btn-sm mt-1"><span class="uil-wallet"></span> Create Store</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
