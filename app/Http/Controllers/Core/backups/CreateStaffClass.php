...
class CreateStaffClass extends Controller{
    ...
    function creatStaff(Request $request,$data){//dd($data);
        ...
        $imgUrl = "public/img/pic.jpg";//$personalInfo["imgUrl"];
     ...
     function workExperience($request){//dd($data['nameOfEstablish'][0]); //dd($data['nameOfEstablish']);       
        $len = sizeof($request->position);//dd($request);
//dd($request->position);
	//dd($request->staffId);
        //dd($len);//$result = false; dd($data['nameOfEstablish'][0]." ".$data['position'][0]." ".$data['start_year'][0]." ".$data['end_year'][0]);
        for($i = 0; $i<$len;$i++){Log::info("insideLoop ".$i);
           try{ $workExperience = new WorkExperience([
                ...
                "jobFunction"=>$request->job_functions[$i],//$data->job_functions[$i],
